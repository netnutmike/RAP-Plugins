...
function aSaveCancelProd() {
	...
	jQuery.post("addons/GIS/autoresponder/product_options_edit.php", {  },
					function(data){
...
function aSaveNewProd() {
	...
	jQuery.post("addons/GIS/autoresponder/product_options_edit.php", { action: 'insert', Name: Name, optionText: optionText, productid: '<?= $_POST['productid'];?>', GroupCode: GroupCode, NewGroupShort: NewGroupShort, NewGroup: NewGroup },
					function(data){
						jQuery('#pr-opt-disp').html(data);
				  	}
				);
...
function aSaveProd(uid) {
	...
	var NewGroupShort =	jQuery("#NewGroupShort").val();
	var NewGroup =	jQuery("#NewGroup").val();
	jQuery.post("addons/GIS/autoresponder/product_options_edit.php", { action: 'update', Name: Name, optionText: optionText, uid: uid, status: status, GroupCode: GroupCode, NewGroupShort: NewGroupShort, NewGroup: NewGroup },
					function(data){
						jQuery('#pr-opt-disp').html(data);
				  	}
				);
...
<? 	if ($_POST['action'] == "edit") {
		...
		echo "<p class=\"georgia-medium\">Edit Auto-Responder Entry</p>";
		} else if ($_POST['action'] == "new") {
			echo "<p class=\"georgia-medium\">Add New Auto-Responder Entry</p>";
		}
?>
...
<tr><td>
 	...
 	<? if ($_POST['action'] == "new") {?>
 	<input type="image" src="/rap_admin/addons/GIS/autoresponder/images/save.png" name="submit" value="Save" onClick="javascript:aSaveNewProd();"/>
 	<? } else {?>
 	<input type="image" src="/rap_admin/addons/GIS/autoresponder/images/save.png" name="submit" value="Save" onClick="javascript:aSaveProd('<?= $grow['uid'];?>');"/>
 	<?  } ?></td><td></td><td align="right"><input type="image" src="/rap_admin/addons/GIS/autoresponder/images/delete48x48.png" name="submit" value="Save" onClick="javascript:aSaveCancelProd();"/></td></tr>
 	<tr><td>&nbsp;</td></tr>
 	<? if ($_POST['action'] == "edit") {?>
 		<tr><td colspan="3">
 		<ul class="checklist">
						<li>
                    		...
                    		<label for="status">Active Auto-Responder Entry</label>
 	...
 	<? 	$sql="select * from g_autoResponderGroups";
		...
		while ($orow = mysql_fetch_array($ors)) { ?>
  			<option value="<?= $orow["GroupCode"]; ?>"<? if ($orow["GroupCode"] == $grow["GroupCode"] ) { echo " SELECTED"; } ?>><?= $orow["GroupName"]; ?></option>
 	...
 	<tr><td>&nbsp;</td></tr>
 	<tr><td class="Prompts" valign="top">Auto-Responder Link:</td><td valign="top"><textarea name="optionText" rows="10" cols="70" id="optionText" ><?= trim($grow['optionText']);?></textarea>
</td></tr>
 	<tr><td>&nbsp;</td></tr>
