...
function aSaveNew() {
	...
	jQuery.post("addons/GIS/autooptin/global_options_edit.php", { action: 'insert', Name: Name, optionText: optionText, GroupCode: GroupCode, NewGroupShort: NewGroupShort, NewGroup: NewGroup },
					function(data){
						jQuery('#gl-opt-disp').html(data);
				  	}
				);
...
function aSave(uid) {
	...
	jQuery.post("addons/GIS/autooptin/global_options_edit.php", { action: 'update', Name: Name, optionText: optionText, uid: uid, status: status, GroupCode: GroupCode, NewGroupShort: NewGroupShort, NewGroup: NewGroup },
					function(data){
						jQuery('#gl-opt-disp').html(data);
				  	}
				);
...
<tr><td>
 	...
 	<? if ($_POST['action'] == "new") {?>
 	<input type="image" src="/rap_admin/addons/GIS/autooptin/images/save.png" name="submit" value="Save" onClick="javascript:aSaveNew();"/>
 	<? } else {?>
 	<input type="image" src="/rap_admin/addons/GIS/autooptin/images/save.png" name="submit" value="Save" onClick="javascript:aSave('<?= $grow['uid'];?>');"/>
 	<?  } ?></td><td></td><td align="right"><input type="image" src="/rap_admin/addons/GIS/autooptin/images/delete48x48.png" name="submit" value="Save" onClick="javascript:aSaveCancel();"/></td></tr>
 	<tr><td>&nbsp;</td></tr>
 	<? if ($_POST['action'] == "edit") {?>
 		<tr><td colspan="3">
 		<ul class="checklist">
