...
function aToggleItem(uid, status, type) {
	...
	jQuery.post("addons/GIS/themes/global_options_edit.php", { uid: uid, action: 'toggle', status: status, type: type },
					function(data){
						jQuery('#gl-opt-disp-set').html(data);
				  	}
				);
...
<? if ($_POST['action'] == "insert" && ($_POST['type'] == '1' || $_POST['type'] == '2')) {
	...
	?>
        ...
        <br><font style="font-size: 14px;"><i>
        		New Record for the Global Theme Options Has Been Inserted...
        		</i><br>&nbsp;
    		</div> 
		</div>
		<br>
		...
		while ($grow = mysql_fetch_array($gid)) { 
			...
			switch ($grow['status']) {
				case '0':
					...
					$toggleaction = "javascript:aToggleItem('" . $grow['uid'] . "', '1', '1')";
					break;
				case '1':
					$ststxt = "Enabled";
					$imgpth = "/rap_admin/addons/GIS/themes/images/green_button32x32.png";
					$toggleaction = "javascript:aToggleItem('" . $grow['uid'] . "', '0', '1')";
					$stsbg = "";
					break;
				case '2':
					$ststxt = "Active";
			...
			<tr class="Prompts" valign="middle"><td><?= $g_timeFormatted ?></td><td>&nbsp;&nbsp;&nbsp;</td><td><?= $grow['template']; ?></td><td>&nbsp;&nbsp;&nbsp;</td><td align="left" <?= $stsbg;?>><a href="#" onClick="<?= $toggleaction;?>"><img src="<?=$imgpth;?>" border="0" width="24" height="24"></a><?= $ststxt; ?></td><td>&nbsp;&nbsp;&nbsp;</td><td><a href="javascript:aEdit('<?= $grow['uid']; ?>')"><img src="addons/GIS/themes/images/edit32x32.png" border="0"></a>&nbsp;&nbsp;<a href="javascript:aDelete('<?= $grow['uid']; ?>')"><img src="addons/GIS/themes/images/delete32x32.png" border="0"></a>&nbsp;</td>
  			</tr>
<? 			}
	} ?>  
  <tr><td>
...
//also update the global type 
	...
	$gid=mysql_query($sql);
	echo "<input type=\"image\" src=\"/rap_admin/addons/GIS/themes/images/save.png\" name=\"submit\" value=\"Save\" onClick=\"javascript:aglTypeSave();\"/>";
} 
