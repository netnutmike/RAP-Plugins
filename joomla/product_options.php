...
function aSave() {
	...
	jQuery.post("addons/GIS/joomla/product_options.php", { PostURL: posturl, productID: prodid, SKU: sku, SecretKey: secretkey },
					function(data){
...
<? if ($_POST['PostURL'] != "" ) {
	...
	if (mysql_num_rows($gid) > 0) {
		...
		$sql = "insert into g_joomlaOptions (post_url, productid, secret_key, SKU, oto) values ('" . $_POST['PostURL'] . "', '" . $productID . "', '" . $_POST['SecretKey'] . "', '" . $_POST['SKU'] . "', '0')";
		$gid=mysql_query($sql);
	}
	...
	var message =	\"Your Product Options for Joomla Have Been Saved!\";
	var errormessage =	\"" . $errormessage . "\";
	var prodid = \"" . $productID . "\"
	jQuery.post(\"addons/GIS/joomla/product_options.php\", { message: message, productID: prodid, errormessage: errormessage }, 
					function(data){ 
						jQuery('#pr-opt-disp').html(data); 
				  	} 
				); 
...
<input type="image" name="submit" src="/rap_admin/addons/GIS/joomla/images/save.png" value="Save" onClick="javascript:aSave();"/> 
</td><td valign="middle">
