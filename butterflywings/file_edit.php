...
function g_getTemplateFolder($productID) {
	...
	$g_prepnd = substr(getcwd(),0,strrpos(getcwd(), "/rap_admin"));
	if ($productID == '-1') 
		$template_path = $g_prepnd . "/rap_admin/addons/GIS/raptools/templates/";
	else
...
function aSave() {
	...
	var page =	"<? echo $_POST['page']; ?>";
	var data =	jQuery("#elm1").val();
	var data = data.replace(/&lt;.\?/g, "\x3C?");
	var data = data.replace(/\?&gt;/g, "?\x3E");
	var action = "1";
	jQuery.post("addons/GIS/raptools/file_edit.php", { pid: '<?= $_POST['pid'];?>', page: page, data: data, action: action },
					function(data){
...
<? 	if ($_POST['pid'] != "" && $_POST['page'] != ""  && $_POST['page'] != "undefined") {
	
	$sourcefile = g_getTemplateFolder($_POST['pid']) . $_POST['page'];
