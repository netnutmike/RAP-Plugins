...
$description="Easily Integrate RAP with WishlistMember Wordpress Add-on";
...
<td align=center>
	<table align=center cellpadding=3 cellspacing=0>
		...
		<?php
			...
			$querystr = "Create table if not exists wishlistmember_options (uid int auto_increment, productid int, post_url varchar(250), secret_key varchar(90), SKU varchar(50), primary key(uid))";
			@mysql_query($querystr);
		?>
		...Done...</h2></td></tr>
			
		<?php
			$sql="SELECT * FROM addons where title='Editor'";
			$result = @mysql_query($sql);
			
			if (mysql_num_rows($result) > 0) {
				$querystr = "insert into tokens (company_id, plugin, tag, pages, description) VALUES ('GIS','WishlistMember','<?=Wishlist_Register() ?>','','Creates Membership in Wishlist Member and returns URL to complete the Registration Process')";
				@mysql_query($querystr);
			} 
		?>	
		<?php
