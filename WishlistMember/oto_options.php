...
function aSaveOTO() {
	...
	jQuery.post("addons/GIS/WishlistMember/oto_options.php", { PostURL: posturl, productID: prodid, SKU: sku, SecretKey: secretkey },
					function(data){
...
<? if ($_POST['PostURL'] != "" ) {
	...
	if (mysql_num_rows($gid) > 0) {
		...
		$sql = "insert into wishlistmember_options (post_url, productid, secret_key, SKU, oto) values ('" . $_POST['PostURL'] . "', '" . $productID . "', '" . $_POST['SecretKey'] . "', '" . $_POST['SKU'] . "', '1')";
		$gid=mysql_query($sql);
	}
	...
	var message =	\"Your OTO Options for Wishlist Member Have Been Saved!\";
	var errormessage =	\"" . $errormessage . "\";
	var prodid = \"" . $productID . "\"
	jQuery.post(\"addons/GIS/WishlistMember/oto_options.php\", { message: message, productID: prodid, errormessage: errormessage }, 
					function(data){ 
						jQuery('#ot-opt-disp').html(data); 
				  	} 
				); 
...
<tr><td>
 <?php 
   ...
   <tr><td><font size="4" color="gray" face=tahoma style="letter-spacing: -1px;">SKU:</font></td><td> <input type="text" name="SKUO" id="SKUO" value="<?php echo $grow['SKU']; ?>"> </td></tr>
   </table>
...
<input type="image" name="submit" src="/rap_admin/addons/GIS/WishlistMember/images/save.png" value="Save" onClick="javascript:aSaveOTO();"/> 
</td><td valign="middle">
