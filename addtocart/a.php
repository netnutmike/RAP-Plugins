...
//
//	Description:	This file is called to create the graphic image for ATC 
//
...
function LoadPNG($imgname){
    ...
    if (trim($_REQUEST['atc']) != "") {
    	$sql="select * from g_addToCart where uid='" . $_REQUEST['atc'] . "'";
		$gid=mysql_query($sql);
		$grow = mysql_fetch_array($gid);
	} else if (trim($_REQUEST['product']) != "") {
		$sql="select * from g_addToCart where productid='" . $_REQUEST['product'] . "'";
		$gid=mysql_query($sql);
		$grow = mysql_fetch_array($gid);
	} else 
		return;
	...
	if ($soldout) {
		if ($grow['endAction'] == '1') {
			...
			//Selling price text
			$text = "Get It Today For Only " . $grow['MoneySymbol'] . $regularprice;
		...
		//	Add colors to text
		$clr = explode('/', $grow['RegularColor']);
		$textcolor = imagecolorallocate($im, $clr[0], $clr[1], $clr[2]);
		$clr = explode('/', $grow['TodayColor']);
		$textcolor2 = imagecolorallocate($im, $clr[0], $clr[1], $clr[2]);
		$clr = explode('/', $grow['SlashColor']);
		$linecolor = imagecolorallocate($im, $clr[0], $clr[1], $clr[2]);
	
		//Selling price text
		$text = "Regular Price " . $grow['MoneySymbol'] . $regularprice . " Today " . $grow['MoneySymbol'] . $todayprice;
		...
		$text = "Regular Price " . $grow['MoneySymbol'];
		$bbox = imagettfbbox($fontsize, 0, $font, $text);
		...
		$x = $x + $bbox[4];
		$text = " Today " . $grow['MoneySymbol'] . $todayprice;
		//$bbox = imagettfbbox($fontsize, 0, $font, $text);
		imagettftext($im, $fontsize, 0, $x, 35, $textcolor, $font, $text);
