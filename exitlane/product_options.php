...
if ($_POST['action'] == "Update" ) {
	...
	$sql = "UPDATE g_ExitLane set offerText='" . $_POST['offerText'] . "', width='" . $_POST['width'] . "', height='" . $_POST['height'] . 
	...
	?>
		<div class="rounded-box-green" id="message-box">
    	    ...
    	    <br><font style="font-size: 18px;"><strong>Good News!</strong></font><img src="/rap_admin/addons/GIS/exitlane/images/info48x48.png" align="right">
        	<br><font style="font-size: 14px;"><i>
        		Exit-Lane Product Options Updated!
        		</i><br>&nbsp;
...
function aSaveatc() {
	...
	jQuery.post("addons/GIS/exitlane/product_options.php", { offerText: offerText, width: width, height: height, x: x, y: y, position: position, horAlign: horAlign,
...
<?
	$sql="select * from g_ExitLane where productID='" . $_POST['productID'] . "'";
	$gid=mysql_query($sql);
	if (mysql_num_rows($gid) == 0) {
		$sql="insert into g_ExitLane (productID, offerText, Status) VALUES ('" . $_POST['productID'] . "', 'Your Text for the Pop Over Goes Here.  It can include HTML, images, etc', '0')";
		$gid=mysql_query($sql);
		$sql="select * from g_ExitLane where productID='" . $_POST['productID'] . "'";
		$gid=mysql_query($sql);
	}
...
<? 		echo "<p class=\"georgia-medium\">Edit Exit-Lane Setup </p>";
...
<tr><td>
 	...
 	<input type="image" src="/rap_admin/addons/GIS/exitlane/images/save.png" name="submit" value="Save" onClick="javascript:aSaveatc();"/><br>&nbsp;&nbsp;Save
...
<tr><td>
  ...
  #elos_CloseIcon {
   background:url(wp-content/plugins/exitlane/images/closebutton2.png) no-repeat;
}</pre><br><br>
  </p></td></tr>
