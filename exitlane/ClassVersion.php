...
// Connect to mysql server
	...
	<form id="registrationform" name="registrationform" method="post" action="/rap_admin/addons/GIS/exitlane/ClassVersion.php">
	<input type="hidden" name="action" value="registration">
	<input type="hidden" name="title" value="<?= $_REQUEST['title']; ?>">
	<table width="500">
	<tr><td>&nbsp;</td></tr>
	<tr><td class="Prompts" colspan="3">Before you can use the the Exit-Lane for RAP addon you need to register it using your paypal email address.  The site owners paypal email will be used for registration.</td></tr>
	<tr><td>&nbsp;</td></tr>
...
class ModVersion
{
	function versioninfo($g_emailaddr)
	{
		...
		if ($reg == '0') { 
			...
			$html="
				...
				<iframe src='addons/GIS/exitlane/ClassVersion.php?action=register&title=" . $title . "' width='800' height='800' frameborder='0'>
	...
	function readAddonsFile($g_emailaddr)
	{
        ...
        $file = "http://rap-tools.com/rap_admin/addons/GIS/regmgr/verify.php?pr=" . $title . "&dn=" . $_SERVER['HTTP_HOST'] . "&pe=" . $g_emailaddr;
        $file = str_replace(" ", "%20", $file);
