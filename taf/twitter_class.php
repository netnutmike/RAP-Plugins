	...
	{ 
		...
		} else 
		{
			if(!empty($status) && strlen($status) <= $this->MaxLength)
			{
				...
				$postfields = "status=". urlencode($status);
				//$postfields = "status=". urlencode(stripslashes(urldecode("testing again and again")));
				//echo "url:" . $url . "\r\n";
				$request = $this->requestToTwitter($url, $postfields);
				return $request;
...
//Request to Twitter with cURL lib
	private function requestToTwitter ( $url='', $postfields="")
	{
		if(empty($this->Username) || empty($this->Password))
		{
			$this->Error(1);
		} else 
		{
			...
			{	
				...
				//echo "http_code:" . $Headers['http_code'] . "\r\n";
				//print_r($Headers);
