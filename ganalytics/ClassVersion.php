...
// Connect to mysql server
	...
	<form id="registrationform" name="registrationform" method="post" action="/rap_admin/addons/GIS/ganalytics/ClassVersion.php">
	<input type="hidden" name="action" value="registration">
	<input type="hidden" name="title" value="<?= $_REQUEST['title']; ?>">
	<table width="500">
	<tr><td>&nbsp;</td></tr>
	<tr><td class="Prompts" colspan="3">Before you can use the the Google Analytics for RAP addon you need to register it using your paypal email address.  The site owners paypal email will be used for registration.</td></tr>
	<tr><td>&nbsp;</td></tr>
 	...
 	<font size="5" color="gray" face=tahoma style="letter-spacing: -1px;">Don't own a copy of Google Analytics for RAP?</font>
 	<P class="Prompts">You can <a href="http://www.rap-tools.com/ganalytics" target="_blank">Click Here</a> to learn more about the add-on and purchase an unlimited use license.</P>
 	<a href="http://www.rap-tools.com/ganalytics" target="_blank"><img src="http://www.rap-tools.com/images/stories/rokstories/gafrap-round-final-small.png" border="0"></a>
<?	
}
...
class ModVersion
{
	function versioninfo($g_emailaddr)
	{
		$title = title;
		$version = version;
	
		$lv = $this->versioncheck($g_emailaddr);
		...
		if ($reg == '0') { 
			...
			$html="
				...
				<iframe src='addons/GIS/ganalytics/ClassVersion.php?action=register&title=" . $title . "' width='800' height='800' frameborder='0'>
	...
	function versioncheck($g_emailaddr){
		$title = title;

		//get the addons
		$line = $this->readAddonsFile($g_emailaddr);
	...
	function readAddonsFile($g_emailaddr)
	{
		global $sys_version;
		$title = title;
        ...
        $file = "http://rap-tools.com/rap_admin/addons/GIS/regmgr/verify.php?pr=" . $title . "&dn=" . $_SERVER['HTTP_HOST'] . "&pe=" . $g_emailaddr . "&rv=" . $sys_version . "&av=" . version;
        $file = str_replace(" ", "%20", $file);
