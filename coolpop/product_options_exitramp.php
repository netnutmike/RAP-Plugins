...
if ($_POST['action'] == "Update" ) {
	...
	$sql = "UPDATE g_CoolPop set offerText='" . $_POST['offerText'] . "', initialPopup='" . $_POST['initialPopup'] . "', continuePopup='" . $_POST['continuePopup'] . "', stayAction='" . $_POST['stayAction'] . "', width='" . $_POST['width'] . "', height='" . $_POST['height'] . 
...
function aSaveatc() {
	...
	jQuery.post("addons/GIS/coolpop/product_options_exitramp.php", { offerText: offerText, initialPopup: initialPopup, continuePopup: continuePopup, stayAction: stayAction, width: width, height: height, x: x, y: y, position: position, horAlign: horAlign,
...
<?
	...
	if (mysql_num_rows($gid) == 0) {
		$sql="insert into g_CoolPop (productID, initialPopup, continuePopup, Status) VALUES ('" . $_POST['productID'] . "', 'If there is text in here it will be displayed in a popup prior to the system popup', 'The text in this option is displayed in the system generated continue message',  '0')";
		$gid=mysql_query($sql);
		$sql="select * from g_CoolPop where productID='" . $_POST['productID'] . "'";
		$gid=mysql_query($sql);
	}
